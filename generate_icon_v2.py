...
def png_to_icns(png_path, icns_path):
    """PNG 转 ICNS (macOS 图标格式)"""
    try:
        img = Image.open(png_path).convert("RGBA")
        # ICNS 必须是正方形，取最小边并居中裁剪
        w, h = img.size
        size = min(w, h)
        left = (w - size) // 2
        top = (h - size) // 2
        ...
        print(f"Successfully created ICNS at {icns_path}")
        return True
    except Exception as e:
        print(f"ICNS Error: {e}")
        return False

if __name__ == "__main__":
    src = r"F:\trae-cn\rust_image_compressor\高速缩图图标.png"
    dst_ico = r"F:\trae-cn\rust_image_compressor\icon.ico"
    dst_icns = r"F:\trae-cn\rust_image_compressor\icon.icns"
    
    if os.path.exists(src):
        create_high_quality_ico(src, dst_ico)
        png_to_icns(src, dst_icns)
    else:
